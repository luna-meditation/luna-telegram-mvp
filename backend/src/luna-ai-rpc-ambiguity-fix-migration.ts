export const lunaAiRpcAmbiguityFixMigration = `
...
begin
  ...
  if p_conversation_id is not null and not exists (
    select 1
    from public.ai_conversations as conversation_row
  ...
  if v_had_existing and (
    ...
    or (v_request.status = 'processing' and v_request.updated_at > now() - interval '3 minutes')
  ) then
    return query select
      v_request.status,
      v_request.quota_charged,
      greatest(0, p_daily_limit - (
        select count(*)::integer
        from public.ai_chat_requests as usage_row
        where usage_row.telegram_id = p_telegram_id
          and usage_row.quota_charged
  ...
  if v_used_count >= p_daily_limit and (not v_had_existing or not v_request.quota_charged) then
    if v_had_existing then
      ...
      set conversation_id = coalesce(p_conversation_id, request_row.conversation_id),
  ...
  return query select v_request.status, v_request.quota_charged,
    greatest(0, p_daily_limit - v_used_count), v_request.attempt_count, true;
end;
$$;
