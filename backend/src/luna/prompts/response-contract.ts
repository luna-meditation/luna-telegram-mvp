...
export const responseContractPrompt = `Return only the structured response contract. recommendedMeditationId, UUIDs, database IDs, URLs, raw JSON, catalog keys, and schema field names must never appear inside message. preferredCatalogKey is a backend-safe catalog key from CATALOG or null; never copy it into visible text.`;
