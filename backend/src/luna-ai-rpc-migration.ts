export const lunaAiRpcMigration = `
...
create index if not exists idx_ai_chat_requests_user_daily
  on public.ai_chat_requests(telegram_id, created_at desc)
  where quota_charged;

drop index if exists public.idx_ai_messages_request_id;
create unique index if not exists idx_ai_messages_request_role
  on public.ai_messages(conversation_id, request_id, role)
  where request_id is not null;

drop index if exists public.idx_ai_usage_request_id;
create unique index if not exists idx_ai_usage_request_id
  on public.ai_usage(telegram_id, request_id);
...
begin
  ...
  if not exists (
    select 1 from public.users where telegram_id = p_telegram_id
  ) then
  ...
  if p_conversation_id is not null and not exists (
    ...
    where id = p_conversation_id and telegram_id = p_telegram_id
  ) then
  ...
  where telegram_id = p_telegram_id and client_request_id = p_client_request_id
  for update;
  had_existing := found;

  if had_existing and (
    existing.status = 'completed'
    or (existing.status = 'processing' and existing.updated_at > now() - interval '3 minutes')
  ) then
    return query select
      existing.status,
      existing.quota_charged,
      greatest(0, p_daily_limit - (
  ...
  if used_count >= p_daily_limit and (not had_existing or not existing.quota_charged) then
    insert into public.ai_chat_requests (
      telegram_id, client_request_id, conversation_id, status, error_code
    ) values (
      p_telegram_id, p_client_request_id, p_conversation_id, 'quota_exhausted', 'quota_exhausted'
    )
  ...
  insert into public.ai_chat_requests (
    telegram_id, client_request_id, conversation_id, status, quota_charged, attempt_count, error_code, updated_at
  ) values (
    p_telegram_id, p_client_request_id, p_conversation_id, 'processing', true, 1, null, now()
  )
  on conflict (telegram_id, client_request_id) do update set
    conversation_id = coalesce(excluded.conversation_id, public.ai_chat_requests.conversation_id),
    status = 'processing',
    quota_charged = true,
    attempt_count = public.ai_chat_requests.attempt_count + 1,
    error_code = null,
    updated_at = now()
  ...
  return query select existing.status, existing.quota_charged,
    greatest(0, p_daily_limit - used_count), existing.attempt_count, true;
end;
$$;
