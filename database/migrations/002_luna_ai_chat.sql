-- Additive Luna AI chat storage. Apply after the base schema.
...
create index if not exists idx_ai_conversations_user_recent
  on public.ai_conversations(telegram_id, last_message_at desc);
create index if not exists idx_ai_messages_conversation_recent
  on public.ai_messages(conversation_id, created_at desc);
create unique index if not exists idx_ai_messages_request_id
  on public.ai_messages(conversation_id, request_id)
  where request_id is not null and role = 'user';
create index if not exists idx_user_memories_active
  on public.user_memories(telegram_id, is_active, updated_at desc);
create index if not exists idx_ai_usage_user_daily
  on public.ai_usage(telegram_id, created_at desc);
...
-- No client policies are intentionally created. All AI data is accessed through
-- the authenticated backend service role, which validates Telegram ownership.
