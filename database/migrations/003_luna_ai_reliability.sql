create table if not exists public.ai_chat_requests (
  ...
  status text not null default 'pending' check (status in (
    'pending', 'processing', 'completed', 'failed_retryable', 'failed_non_retryable', 'quota_exhausted'
  )),
  ...
  if had_existing and (existing.status = 'completed' or (existing.status = 'processing' and existing.updated_at > now() - interval '3 minutes')) then
    return query select existing.status, existing.quota_charged,
      greatest(0, p_daily_limit - (
        select count(*)::integer from public.ai_chat_requests
        where telegram_id = p_telegram_id and quota_charged and created_at >= date_trunc('day', now() at time zone 'utc') at time zone 'utc'
